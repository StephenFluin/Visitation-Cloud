...
function TagCloud(cntr,sstyle,carray,chighlight,urlstub)
{
	...
	function draw()
	{
		...
		scale = 100 / (Math.sqrt(maxsize / Math.PI) * 2)
	...
	function itDraw()
	{
		console.log("Calling itdraw");
		var nd = drawQueue.shift()
		if(nd)
		{
			console.log("about to draw a circle.");
			drawCircle(nd)
			timeout = setTimeout(itDraw, 20)
		} else {
			console.log("no new tags in queue to draw.");
		}
		console.log("done call itdraw");
	}
	
	function drawCircle(nd)
	{
		console.log("Starting to draw a circle.");
		...
		if(drawn.length == 0)
		{
			...
			console.log("Checking for collission, radius is " + checkradius + " and collision is " + collision + ".");
			while(!collision && checkradius)
			{
